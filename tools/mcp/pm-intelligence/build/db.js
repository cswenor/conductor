...
export async function getPmDir() {
    const root = await getRepoRoot();
    return join(root, ".pm");
}
// ─── Schema Migrations ──────────────────────────────────
...
export const VALID_PRIORITIES = ["critical", "high", "normal"];
...
export async function getIssuesByWorkflow(workflow) {
    const db = await getDb();
    const rows = db
...
export async function getLocalBoardSummary() {
    const db = await getDb();
    // Counts by workflow (open issues only)
    const workflowCounts = db
        .prepare("SELECT workflow, COUNT(*) as count FROM issues WHERE state = 'open' GROUP BY workflow")
        .all();
    const byWorkflow = {};
    ...
    const priorityCounts = db
        .prepare("SELECT priority, COUNT(*) as count FROM issues WHERE state = 'open' GROUP BY priority")
        .all();
    const byPriority = {};
    ...
    for (const [issueNum, blockers] of blockedMap) {
        ...
        if (issue)
            blockedIssues.push({ issue, blockedBy: blockers });
    }
    // Total open issues
    const totalRow = db
    ...
    if (active > 1)
        healthScore -= (active - 1) * 15;
    const rework = byWorkflow["Rework"] || 0;
    if (rework > 0)
        healthScore -= rework * 10;
    const review = byWorkflow["Review"] || 0;
    if (review > 3)
        healthScore -= (review - 3) * 5;
    const backlog = byWorkflow["Backlog"] || 0;
    if (total > 0 && backlog / total > 0.5)
        healthScore -= 10;
    if (blockedIssues.length > 0)
        healthScore -= Math.min(blockedIssues.length * 5, 20);
...
export async function moveIssueWorkflow(issueNumber, targetState, actor = "claude") {
    const db = await getDb();
    const issue = db
    ...
    const currentState = issue.workflow;
    ...
    if (targetState === "Active") {
        ...
        if (activeCount.count >= 1) {
            const activeIssue = db
                ...
                .get(issueNumber);
            throw new Error(`WIP limit reached: #${activeIssue.number} "${activeIssue.title}" is already Active. ` +
                `Move it to Review or Done first.`);
        }
    }
    // Update workflow
    db.prepare("UPDATE issues SET workflow = ? WHERE number = ?").run(targetState, issueNumber);
    // If moving to Done and issue is open on GitHub, mark closed locally
    if (targetState === "Done") {
        db.prepare("UPDATE issues SET state = 'closed', closed_at = datetime('now') WHERE number = ? AND state = 'open'").run(issueNumber);
    }
    // Record event
    db.prepare(`
...
export async function addDependency(blockerIssue, blockedIssue, depType = "blocks") {
    const db = await getDb();
    ...
    if (wouldCycle) {
        throw new Error(`Adding dependency #${blockerIssue} → #${blockedIssue} would create a cycle`);
    }
    db.prepare(`
  ...
  `).run(blockedIssue, null, String(blockerIssue), JSON.stringify({ dep_type: depType, blocker: blockerIssue }));
}
/** Check if adding an edge would create a cycle (DFS) */
function checkForCycle(db, from, to) {
    ...
    while (stack.length > 0) {
        ...
        visited.add(current);
        const deps = db
            .prepare("SELECT blocked_issue FROM dependencies WHERE blocker_issue = ? AND resolved_at IS NULL")
            .all(current);
...
export async function getDependencies(issueNumber) {
    const db = await getDb();
    const blockedBy = db
        .prepare("SELECT blocker_issue, dep_type, resolved_at FROM dependencies WHERE blocked_issue = ?")
        .all(issueNumber);
    const blocks = db
        .prepare("SELECT blocked_issue, dep_type, resolved_at FROM dependencies WHERE blocker_issue = ?")
        .all(issueNumber);
    return {
...
export async function getCycleTimes(days = 90) {
    const db = await getDb();
    ...
    for (const { issue_number } of doneEvents) {
      ...
      `)
        ...
        if (firstActive && lastDone) {
            const hours = (new Date(lastDone.timestamp).getTime() -
...
export async function upsertIssue(data) {
    ...
    if (existing) {
      ...
      WHERE number = ?
    `).run(data.title, data.body, data.state, data.author, data.updated_at, data.closed_at, data.number);
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)
    `).run(data.number, data.title, data.body, data.state, data.author, data.created_at, data.updated_at, data.closed_at, workflow);
    }
    // Sync labels
    db.prepare("DELETE FROM issue_labels WHERE issue_number = ?").run(data.number);
    const insertLabel = db.prepare("INSERT INTO issue_labels (issue_number, label) VALUES (?, ?)");
    for (const label of data.labels) {
        insertLabel.run(data.number, label);
    }
    // Sync assignees
    db.prepare("DELETE FROM issue_assignees WHERE issue_number = ?").run(data.number);
    const insertAssignee = db.prepare("INSERT INTO issue_assignees (issue_number, login) VALUES (?, ?)");
...
export async function upsertPR(data) {
  ...
  `).run(data.number, data.title, data.state, data.author, data.branch, data.base_branch, data.created_at, data.updated_at, data.merged_at, data.closed_at, data.additions, data.deletions, data.changed_files, data.review_state, data.draft ? 1 : 0);
    // Sync issue links
    db.prepare("DELETE FROM pr_issue_links WHERE pr_number = ?").run(data.number);
    const insertLink = db.prepare("INSERT OR IGNORE INTO pr_issue_links (pr_number, issue_number, link_type) VALUES (?, ?, ?)");
    for (const link of data.linked_issues) {
        insertLink.run(data.number, link.issue_number, link.link_type);
    }
}
/** Record last sync timestamp */
export async function updateSyncState(resource, cursor) {
    const db = await getDb();
    db.prepare(`
