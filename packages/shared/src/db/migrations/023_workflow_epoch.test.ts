/**
 * Migration 023 test: workflow_epoch column
 *
 * Verifies that:
 * - workflow_epoch column exists on runs table
 * - It defaults to 0
...
import { migration023 } from './023_workflow_epoch.ts';
...
describe('migration 023: workflow_epoch', () => {
  it('creates workflow_epoch column on runs with INTEGER type', () => {
    ...
    const col = columns.find(c => c.name === 'workflow_epoch');
  ...
  it('defaults workflow_epoch to 0 for new rows', () => {
    applyMigrationsUpTo(db, 23);

    const { runId } = seedProjectAndRun(db);
    const run = db.prepare('SELECT workflow_epoch FROM runs WHERE run_id = ?').get(runId) as { workflow_epoch: number };
    expect(run.workflow_epoch).toBe(0);
  });

  it('existing data survives migration upgrade with default epoch 0', () => {
    // Apply up to migration 22 and seed data before 023
    ...
    // Verify original data intact and epoch defaulted to 0
