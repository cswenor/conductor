...
export { redactSecretPatterns, SECRET_PATTERNS } from '../utils/redact.ts';
...
// Secret redaction is now in ../utils/redact.ts
...
export function assembleContext(
  ...
  // Attach rewind context summary if present on the run
  if (run.rewindContextSummary !== undefined) {
    context.rewindContextSummary = run.rewindContextSummary;
  }
...
export function formatContextForPrompt(context: AgentContext, budgets?: SectionBudgets): string {
  ...
  // Rewind context section (prior work summary from checkpoint rewind)
  if (context.rewindContextSummary !== undefined) {
    sections.push('');
    sections.push('## Prior Context (from rewind)');
    sections.push('This run was rewound to an earlier checkpoint. Summary of prior work:');
    sections.push(context.rewindContextSummary);
  }
