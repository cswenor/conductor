...
/**
 * Push the run's branch to GitHub using an installation token.
 *
 * Called after the code reviewer approves. Obtains a short-lived token via
 * resolveCredentials, constructs a one-shot auth URL, and pushes. The auth URL
 * and token are never written to logs or persisted in error messages.
 */
async function handlePrCreation(
  ...
  // Re-read run and bail if already terminal
  const freshRun = getRun(db, runId);
  if (freshRun === null || TERMINAL_PHASES.has(freshRun.phase)) {
    log.info({ runId }, 'Run is null or in terminal phase, skipping PR creation');
    return;
  }
